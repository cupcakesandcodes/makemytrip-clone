...
const Flightdetail = () => {
  ...
  // ✅ If no data, show fallback UI instead of crashing
  if (!res) {
    return (
      <div style={{ textAlign: "center", marginTop: "50px", color: "gray" }}>
        <h2>No flight data found 🛫</h2>
        <p>Please select a flight before proceeding to checkout.</p>
      </div>
    )
  }
  ...
  const getDuration = () => {
    try {
      const depTime = res.departure.scheduled.split("T")[1].split("+")[0]
      const arrTime = res.arrival.scheduled.split("T")[1].split("+")[0]
      const dep = depTime.split(":").map(Number)
      const arr = arrTime.split(":").map(Number)
        ...
        <div className="flightdetailheader">
          <div>
            ...
            <p>
              ...
              <span className="date">Non Stop {getDuration()}</span>
            </p>
          </div>
          <div></div>
          <div>
          ...
          <div id="flight_timer" className="flight_time">
            <div>
              <div className="flight_time">
                <h4>
                  {res.departure?.scheduled?.split("T")[1]?.split("+")[0]}
                </h4>
